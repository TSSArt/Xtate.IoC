...
public class NamedPipeIoProcessor() : IoProcessorBase(@"http://www.w3.org/TR/scxml/#NamedPipeEventProcessor", @"net.pipe")
{
	...
	public required IInternalEventDispatcher<NamedPipeIoProcessor> InternalEventDispatcher { private get; [SetByIoC] init; }
	...
	protected override ValueTask Dispatch(IRouterEvent routerEvent, CancellationToken token)
	{
		...
		if (!NamedPipeController.TryParseTarget(target, out var host, out var name, out var serviceId))
		{
		...
		return NamedPipeController.SendEvent(host, name, serviceId, routerEvent, token);
	}

	protected override FullUri CreateExternalServiceTarget(InvokeId invokeId) => NamedPipeController.ToInvokeTarget(invokeId);

	protected override FullUri CreateStateMachineTarget(SessionId sessionId) => NamedPipeController.ToSessionTarget(sessionId);
}