...
internal sealed class StateMachineHostPersistedContext : StateMachineHostContext
{
	...
	{
		public InvokedServiceMeta(in Bucket bucket) : this(
			bucket.GetSessionId(Key.ParentSessionId) ?? throw new PersistenceException(Resources.Exception_MissedParentSessionId),
			bucket.GetInvokeId(Key.InvokeId) ?? throw new PersistenceException(Resources.Exception_InvokedServiceMetaMissedInvokeId), bucket.GetSessionId(Key.SessionId)) { }
