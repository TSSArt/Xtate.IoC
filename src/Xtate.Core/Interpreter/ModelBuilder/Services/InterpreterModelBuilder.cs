...
public class InterpreterModelBuilder : StateMachineVisitor
{
	...
	protected override void Build(ref StateMachineEntity stateMachineProperties)
	{
		...
		{
			var firstState = stateMachineProperties.States[0].UseAncestor.As<StateEntityNode>();
			var transition = TransitionNodeFactory(transitionNodeDocumentId, new TransitionEntity { Target = [firstState.Id] });
			stateMachineProperties.Initial = InitialNodeFactory(initialNodeDocumentId, new InitialEntity { Transition = transition });
			_targetMap.Add(transition);
		}
		else
		{
	...
	protected override void Build(ref StateEntity stateProperties)
	{
		...
		stateProperties.Id ??= Identifier.New();

		base.Build(ref stateProperties);

		if (!stateProperties.States.IsDefaultOrEmpty && stateProperties.Initial is null)
		{
			var firstState = stateProperties.States[0].UseAncestor.As<StateEntityNode>();
			var transition = TransitionNodeFactory(transitionNodeDocumentId, new TransitionEntity { Target = [firstState.Id] });
			stateProperties.Initial = InitialNodeFactory(initialNodeDocumentId, new InitialEntity { Transition = transition });
			_targetMap.Add(transition);
		}
		else
		{
	...
	protected override void Build(ref ParallelEntity parallelProperties)
	{
		parallelProperties.Id ??= Identifier.New();

		base.Build(ref parallelProperties);
	}

	protected override void Build(ref HistoryEntity historyProperties)
	{
		historyProperties.Id ??= Identifier.New();

		base.Build(ref historyProperties);
	}

	protected override void Build(ref FinalEntity finalProperties)
	{
		finalProperties.Id ??= Identifier.New();

		base.Build(ref finalProperties);
	}
