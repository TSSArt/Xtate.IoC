...
#pragma warning restore IDE0028
	...
	public static EventName FromString(string? name)
	{
		...
		var buf = new IIdentifier[GetCount(name)];
	...
	private static EventName GetEventName(IIdentifier id1, IIdentifier id2, string name)
	{
		...
		var buf = new IIdentifier[2 + count];
	...
	public bool IsMatchedToEventDescriptor(string eventDescriptor)
	{
		...
		{
			var dotIndex = descriptor.IndexOf('.');
			var descriptorToken = dotIndex >= 0 ? descriptor[..dotIndex] : descriptor;

			if (!descriptorToken.Equals(part.Value.AsSpan(), StringComparison.Ordinal))
			{
				return false;
			}
			...
			descriptor = descriptor[(dotIndex + 1)..];
		}
