...
public partial class DataModelList
{
	public readonly struct KeyEnumerable : IEnumerable<string>
	{
		...
		internal KeyEnumerable(DataModelList list) => _list = list;

	#region Interface IEnumerable

		IEnumerator IEnumerable.GetEnumerator() => new KeyEnumerator(_list);

	#endregion

	#region Interface IEnumerable<string>

		IEnumerator<string> IEnumerable<string>.GetEnumerator() => new KeyEnumerator(_list);

	#endregion

		public KeyEnumerator GetEnumerator() => new(_list);
	}

	public struct KeyEnumerator : IEnumerator<string>
	{
		private KeyValuePairEnumerator _enumerator;

		[SuppressMessage(category: "ReSharper", checkId: "NotDisposedResourceIsReturned")]
		internal KeyEnumerator(DataModelList list) => _enumerator = list.KeyValuePairs.GetEnumerator();

		public readonly string Current => _enumerator.Current.Key;
	...
	public struct ValueEnumerator : IEnumerator<DataModelValue>
	{
		...
		internal ValueEnumerator(DataModelList list)
		{
			...
			_args.Index = -1;
			Current = DataModelValue.Undefined;
		}
		...
		public bool MoveNext()
		{
			if (_args.Index < _list._count)
			{
				...
				if (_args.Key is not null)
				{
					...
					Current = result ? _args.Value : DataModelValue.Undefined;
