...
public static class DataModelConverter
{
	...
	public static string ToJson(DataModelValue value, JsonOptions options = JsonOptions.None) => JsonSerializer.Serialize(value, GetOptions(options));

	public static byte[] ToJsonUtf8Bytes(DataModelValue value, JsonOptions options = JsonOptions.None) => JsonSerializer.SerializeToUtf8Bytes(value, GetOptions(options));

	public static void ToJson(Stream stream,
							  DataModelValue value,
							  JsonOptions options = JsonOptions.None) =>
		JsonSerializer.Serialize(stream, value, GetOptions(options));
	...
	public static string ToXml(DataModelValue value, XmlOptions options = XmlOptions.None) => XmlConverter.ToXml(value, (options & XmlOptions.WriteIndented) != 0);

	public static byte[] ToXmlUtf8Bytes(DataModelValue value, XmlOptions options = XmlOptions.None)
	{
	...
	public static Task ToXmlAsync(Stream stream,
								  DataModelValue value,
								  XmlOptions options = XmlOptions.None,
								  CancellationToken token = default) =>
		XmlConverter.AsXmlToStreamAsync(value, (options & XmlOptions.WriteIndented) != 0, stream.InjectCancellationToken(token));

	public static void ToXml(Stream stream, DataModelValue value, XmlOptions options = XmlOptions.None) => XmlConverter.AsXmlToStream(value, (options & XmlOptions.WriteIndented) != 0, stream);
