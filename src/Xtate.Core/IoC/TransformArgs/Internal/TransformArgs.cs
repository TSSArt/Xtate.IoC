...
public readonly struct TransformArgs<T, TArg, TNewArg>
{
	...
	public TransformArgs(bool synchronous, IServiceCollection services, Delegate transform)
	{
		...
		if (services.IsRegistered(TypeKey.ImplementationKey<ServiceFactoryNewArgsAsync<T, TArg, TNewArg>, Empty>()))
		{
			return;
		}
		...
		else if (!services.IsRegistered(TypeKey.ImplementationKey<ServiceFactoryNewArgsSync<T, TArg, TNewArg>, Empty>()))
		{
