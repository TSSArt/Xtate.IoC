...
public class DiTest2
{
    ...
    {
        public Class1 PropClass1 { [UsedImplicitly] get; } = class1;
    ...
    #region Interface IAsyncInitialization
        ...
        {
            PropClass2 = await class2Factory();
            PropClass4 = await class4Factory(arg);

            await foreach (var i in forDecorations)
            {
                PropForDecorations = PropForDecorations.Add(i);
            }

            foreach (var i in forDecorationsSync)
            {
                PropForDecorationsSync = PropForDecorationsSync.Add(i);
            }

            await foreach (var i in class4Factories(arg))
            {
                PropClass4S = PropClass4S.Add(i);
            }

            foreach (var i in class4FactoriesSync(arg))
            {
                PropClass4SSync = PropClass4SSync.Add(i);
            }

            await foreach (var i in class4FactoriesTwo(arg, arg))
            {
                PropClass4STwo = PropClass4STwo.Add(i);
            }

            foreach (var i in class4FactoriesTwoSync(arg, arg))
            {
