...
public class AncestorTest
{
	...
	private class LazyImpl<T> : ILazy<T>, IAsyncInitialization
	{
		...
		public LazyImpl(ServiceProviderActions actions, Func<ValueTask<T?>> factory) => _valueTask = actions.TryCapture(this) ? default : factory().Preserve();

	#region Interface IAsyncInitialization

		ValueTask IAsyncInitialization.InitializeAsync() => _valueTask.IsCompletedSuccessfully ? ValueTask.CompletedTask : new ValueTask(_valueTask.AsTask());
	...
	#region Interface ILazy<T>
		...
		private T? GetValue() => _valueTask.Result;

		internal void SetValue(T? instance) => _valueTask = new ValueTask<T?>(instance);
	}
