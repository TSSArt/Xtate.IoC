...
public class TokenAndLazyValueCoverageTest
{
	...
	public void InvokeIdSupportsGeneratedExplicitAndUniqueIdentityPaths()
	{
		...
		var explicitInvokeId = InvokeId.New(stateId, invokeId: "invoke");
		...
		Assert.IsFalse(string.IsNullOrWhiteSpace(generated.Value));
		Assert.IsTrue(generated.Value.StartsWith(value: "state.", StringComparison.Ordinal));
	}
	...
	public void DisposeTokenWrapsCancellationTokenEqualityAndCancellationBehavior()
	{
		...
		different.ThrowIfCancellationRequested();
	...
	public void LazyValueFactoriesEvaluateOnceAndPreserveArguments()
	{
		...
		Assert.AreEqual(expected: "created", noArg.AsString());
		Assert.AreEqual(expected: "created", noArg.AsString());
		Assert.AreEqual(expected: 1, noArgCalls);

		var oneArg = LazyValue.Create(arg: "arg", arg => new DataModelValue(arg + "-value"));
		Assert.AreEqual(expected: "arg-value", oneArg.AsString());
