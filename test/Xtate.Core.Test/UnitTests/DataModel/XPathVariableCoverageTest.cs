...
public class XPathVariableCoverageTest
{
	...
	public async Task InitializedDescriptorDelegatesVariableCreationToEngine()
	{
		...
		Assert.AreEqual(expected: "created", current.Name);
	}
	...
	public async Task CompiledExpressionExposesMetadataAndInitializesContextBeforeReturningExpression()
	{
		...
		var compiled = new XPathCompiledExpression(expression: "1 + 1", xmlNamespacesInfo: null, _ => context);

		Assert.AreEqual(expected: "1 + 1", compiled.Expression);
		...
		Assert.AreEqual(expected: "1 + 1", first.Expression);
	}
}