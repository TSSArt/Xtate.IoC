...
public class SharedMemoryStreamsCoverageTest
{
	...
	public async Task ReadWriteStreamForwardsSynchronousAndAsynchronousOperations()
	{
		...
		Assert.AreEqual(expected: 7, stream.Length);
		Assert.AreEqual(expected: 7, stream.Position);
		...
		Assert.AreEqual(expected: 1, await stream.ReadAsync(buffer, offset: 0, count: 1, CancellationToken.None));
		Assert.AreEqual(expected: 6, buffer[0]);
		Assert.AreEqual(expected: 1, await stream.ReadAsync(buffer, offset: 0, count: 1, CancellationToken.None));
		Assert.AreEqual(expected: 7, buffer[0]);
		...
		stream.SetLength(3);
		Assert.AreEqual(expected: 3, stream.Length);
		stream.GetType().GetMethod(name: "Dispose", BindingFlags.Instance | BindingFlags.NonPublic)!.Invoke(stream, [false]);
		Assert.IsTrue(stream.CanRead);
	}
	...
	public void SharedMemoryStreamsEnforcesReaderWriterExclusionAndDeletion()
	{
		...
		missingReader.GetType().GetMethod(name: "Dispose", BindingFlags.Instance | BindingFlags.NonPublic)!.Invoke(missingReader, [false]);
		...
		Assert.AreEqual(expected: 3, reader1.Length);
		Assert.AreEqual(expected: 1, reader1.ReadByte());
		Assert.AreEqual(expected: 1, reader2.ReadByte());
		Assert.ThrowsExactly<IOException>([ExcludeFromCodeCoverage]() => streams.OpenWrite("key"));
		Assert.ThrowsExactly<IOException>([ExcludeFromCodeCoverage]() => streams.Delete("key"));
	...
	public void DisposedReadWriteStreamRejectsOperations()
	{
		...
		Assert.ThrowsExactly<ObjectDisposedException>([ExcludeFromCodeCoverage]() => stream.Seek(offset: 0, SeekOrigin.Begin));
		Assert.ThrowsExactly<ObjectDisposedException>([ExcludeFromCodeCoverage]() => stream.SetLength(0));
		Assert.ThrowsExactly<ObjectDisposedException>([ExcludeFromCodeCoverage]() => stream.Read(new byte[1], offset: 0, count: 1));
		Assert.ThrowsExactly<ObjectDisposedException>([ExcludeFromCodeCoverage]() => stream.Write(new byte[1], offset: 0, count: 1));
	}
	...
	public void DeleteAllRemovesOnlyMatchingFreeStreams()
	{
		...
		Assert.AreEqual(expected: 2, streams.DeleteAll(key => key.Partition == "first"));
	...
	public async Task InMemoryStorageProviderCreatesAndRemovesPartitionedStorage()
	{
		...
		Assert.AreSame(transactionalStorage, await provider.GetTransactionalStorage(partition: "partition", key: "key"));
		Assert.IsNotNull(capturedStream);
		capturedStream.Dispose();
		await provider.RemoveTransactionalStorage(partition: "partition", key: "key");
