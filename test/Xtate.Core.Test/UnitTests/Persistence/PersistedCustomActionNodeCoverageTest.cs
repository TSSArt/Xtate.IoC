...
public class PersistedCustomActionNodeCoverageTest
{
	...
	public async Task PersistedCustomActionNodeForwardsAndStoresActionProperties()
	{
		...
		Assert.AreEqual("urn:test-actions", node.XmlNamespace);
		Assert.AreEqual("action", node.XmlName);
		Assert.AreEqual("<action xmlns='urn:test-actions'/>", node.Xml);
		Assert.IsTrue(node.Locations.IsDefault);
		Assert.IsTrue(node.Values.IsDefault);
		Assert.AreEqual(42, node.DocumentId);
		Assert.AreEqual(42, node.DocumentId);
		...
		Assert.AreEqual(42, bucket.GetInt32(Key.DocumentId));
		Assert.AreEqual("urn:test-actions", bucket.GetString(Key.Namespace));
		Assert.AreEqual("action", bucket.GetString(Key.Name));
		Assert.AreEqual("<action xmlns='urn:test-actions'/>", bucket.GetString(Key.Content));
